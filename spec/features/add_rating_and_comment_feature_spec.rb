...
feature "Adding comment feature" do
  ...
  scenario "a logged in user can submit the comment form" do
    visit new_review_comment_path(review.id)
    fill_in('Add Comment', with: "this review makes no sense")
    click_button("Add Comment!")
    expect(page).to have_current_path movie_path(review.movie.id)
  end
end

feature "Adding rating feature" do
  let(:review) { Review.new(title: "It was okay", body: "somebody") }
  before(:each) do
    user = User.create!(username:"vi", password:"12345678")
    movie = Movie.create!(title: "Dumb & Dumber", description: "A very funny movie", director: "John Smith", release_date: "1992-11-18")
  ...
  scenario "a logged in user can go to the ratings form" do
    visit new_review_rating_path(review.id)
    expect(page).to have_content "Rate this review:"
  end

  scenario "a logged out user cannot go to the ratings form" do
    visit '/'
    click_link "Logout"
    visit new_review_rating_path(review.id)
    expect(page).to have_current_path no_access_path
  ...
  scenario "a logged in user can submit the ratings form" do
    visit new_review_rating_path(review.id)
    choose('4')
    click_button("Rate!")
    expect(page).to have_current_path movie_path(review.movie.id)
